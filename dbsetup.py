...
def create_session(c, data):
    sql = ''' INSERT INTO sessions(ip, os, browser, session, created_at)
              VALUES (?,?,?,?,?) '''
    c.execute(sql, data)
...
def user_visits(c, session_id):
    sql = "SELECT visits FROM pages where session =?"
    c.execute(sql,[session_id])
    rows = c.fetchall()
    return rows

def dash(c, session_id):
    sql = "SELECT created_at, ip, os, browser FROM sessions where session =? "
    c.execute(sql,[session_id])
    rows = c.fetchall()
    return rows
